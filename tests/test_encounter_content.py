...
def test_load_scene_parses_optional_encounter_block() -> None:
    scene_path = Path("sample_game/scenes/test_encounter")

    scene = load_scene(scene_path)

    assert scene.id == "test_encounter"
    assert scene.type == "encounter"
    assert scene.encounter is not None
    assert scene.encounter.grid.width == 8
    assert scene.encounter.grid.height == 8
    ...
    assert scene.encounter.victory.next_scene == "test_encounter_victory"
    assert scene.encounter.defeat is not None
    assert scene.encounter.defeat.next_scene == "test_encounter_defeat"
    assert scene.encounter.flee is not None
    assert scene.encounter.flee.allowed is True
    assert scene.encounter.flee.next_scene == "test_encounter_intro"
