...
def test_encounter_scene_generates_runtime_actions_and_grid() -> None:
...
def test_encounter_turn_advances_enemy_behaviors_after_player_move() -> None:
    session = Game(str(SAMPLE_GAME_DIR)).create_session()
    session.current_scene_id = "test_encounter"
...
def test_encounter_attack_can_end_scene_with_victory(monkeypatch) -> None:
    session = Game(str(SAMPLE_GAME_DIR)).create_session()
    session.current_scene_id = "test_encounter"
...
def test_save_and_load_preserve_encounter_progress(tmp_path: Path) -> None:
    ...
    assert loaded.current_scene_id == "test_encounter"
