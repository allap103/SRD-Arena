...
class CapabilityExecutionStatistics:
    """Actor statistics required to resolve a capability."""
...
class CapabilityExecutionContext:
    """Provider-neutral inputs required to execute a capability definition."""
...
def resolve_capability(
    ...
    """Resolve an executable capability against its selected targets."""
    return _resolve_capability(context)


def _resolve_capability(context: CapabilityExecutionContext) -> CapabilityActionResult:
    ...
    attack_bonus = (
        _attack_modifier(resolution, context.statistics)
        if isinstance(resolution, AttackResolution)
        else context.statistics.attack_bonus
    )
    save_ability = (
    ...
    target_suffix = (
        ...
        if definition.target.kind == "creature" and len(targets) == 1
        else ""
    )
    messages = [
        (
            "system",
            f"{context.creature.name} {context.activation_verb} "
            f"{context.capability_name}{target_suffix}.",
        )
    ]
    ...
    levels_above = resource_level - context.base_resource_level
    if levels_above > 0:
        scaled: list[_CapabilityDamage] = []
        for damage in damage_definitions:
            resource_damage_increment = _resource_dice_increment(
                definition,
                "damage_dice",
                damage.damage_type,
            )
            if resource_damage_increment is None:
                scaled.append(damage)
                continue
            increment_count, increment_sides = _parse_damage_dice(
                resource_damage_increment
            )
    ...
    affected_targets: list[CapabilityTargetContext] = []
    for target in targets:
        ...
        if isinstance(resolution, SavingThrowResolution):
            ...
            automatic_success_reasons = tuple(
                f"{context.capability_name}: immune to {condition}"
            ...
            automatic_success_reasons += tuple(
                f"{context.capability_name}: {trait}"
            ...
            if creature_type in automatic_failure_creature_types:
                automatic_reasons += (f"{context.capability_name}: {creature_type}",)
        ...
        if not context.removable_conditions:
            if automatic_success_reasons:
                messages.append(
                    (
                        "system",
                        f"{target.target_label} is unaffected by {context.capability_name}: "
                ...
                messages.append(
                    (
                        "system",
                        f"{target.target_label} resists {context.capability_name} with a "
                ...
                messages.append(
                    (
                        "system",
                        f"{context.capability_name} {outcome} {target.target_label}.",
                    )
                )
    ...
    if context.removable_effect_kinds:
        for target in affected_targets:
            selected_removal = context.selected_condition
            if (
                context.remove_effect_selection == "all"
                and context.removable_conditions
            ):
                target_removed_conditions = tuple(
                    condition
                    for condition in context.removable_conditions
            ...
            elif selected_removal in context.removable_conditions:
            ...
            if "curse" in context.removable_effect_kinds and (
                context.remove_effect_selection == "all"
                or (
                ...
                effects.append(
                    EffectResult(
                        ...
                        data={
                            "effect_kind": "curse",
                            "effect_id": effect_id,
                            "all": context.remove_effect_selection == "all",
                        },
                    )
                )
                messages.append(("system", f"A curse ends on {target.target_label}."))
            if (
                "hit_point_maximum_reduction" in context.removable_effect_kinds
    ...
    return CapabilityActionResult(
        ...
        details={
            ...
            "base_resource_level": context.base_resource_level,
            "resource_level": resource_level,
            **context.result_metadata,
...
def _resolve_follow_up(
    ...
    definition = context.definition
    if resource_level > context.base_resource_level:
        scaled: list[_CapabilityDamage] = []
        for damage in damage_definitions:
...
def _scaled_damage_dice(
    ...
    count, sides = _parse_damage_dice(dice)
    if sides != increment_sides:
        raise ValueError("Resource damage scaling must use the base damage die.")
